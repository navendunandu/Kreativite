<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="Start your development with Meyawo landing page.">
    <meta name="author" content="Devcrud">
    <title>KREATIVITE</title>
    <!-- font icons -->
    <link rel="stylesheet" href="../Assets/Templates/Main/vendors/themify-icons/css/themify-icons.css">
    <!-- Bootstrap + Meyawo main styles -->
	<link rel="stylesheet" href="../Assets/Templates/Main/css/meyawo.css">
    <style>
        #mb{
            min-height: 100vh;
            padding-top: 130px !important;
        }
    </style>
</head>
<body data-spy="scroll" data-target=".navbar" data-offset="40" id="home">

    <!-- Page Navbar -->
    <nav class="custom-navbar" data-spy="affix" data-offset-top="20">
        <div class="container">
            <a class="logo" href="#">KREATIVITE</a>         
            <ul class="nav">
                <li class="item">
                    <a class="link" href="./Homepage.php">Home</a>
                </li>
                <li class="item">
                    <a class="link" href="MyProfile.php">My Profile</a>
                </li>
                <li class="item">
                    <a class="link" href="ViewProject.php">New Projects</a>
                </li>
                <li class="item">
                    <a class="link" href="PreviousWork.php">My Works</a>
                </li>
                <li class="item">
                    <a class="link" href="ViewApplication.php">New Applications</a>
                </li>
                <!-- <li class="item ml-md-3">
                    <a href="components.html" class="btn btn-primary">Components</a>
                </li> -->
            </ul>
            <a href="javascript:void(0)" id="nav-toggle" class="hamburger hamburger--elastic">
                <div class="hamburger-box">
                  <div class="hamburger-inner"></div>
                </div>
            </a>
        </div>          
    </nav><!-- End of Page Navbar -->
    <section class="section pt-0" id="mb">